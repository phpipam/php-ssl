...
$select = "SELECT c.id, c.cn, c.sans, c.csr_pem, c.key_algo, c.key_size, c.status, c.created, c.t_id, c.cert_id, c.renewed_by,
         ...
         . ($user->admin !== "1" ? " WHERE c.t_id = " . (int)$user->t_id : "")
...
<div class="page-header">
	<h2 class="page-title"><?php print $url_items['csrs']['icon']." "._("CSR Requests"); ?></h2>
	<hr>
</div>

<p class='text-secondary'><?php print _("Generate and manage Certificate Signing Requests."); ?></p>
...
<div style="margin-bottom:10px">
	...
	<a href="/route/modals/csrs/import.php"
...
if (empty($groups)) {
    ...
    foreach ($groups as $tenant_id => $csrs) {
        ...
        if (empty($csrs)) {
            print "<tr><td colspan='7'><div class='alert alert-info py-2 mb-0'>" . _("No CSRs for this tenant.") . "</div></td></tr>";
            continue;
        }
        ...
        foreach ($csrs as $c) {
            ...
            if ($has_key) {
                ...
                $actions .= "<a class='btn btn-sm bg-red-lt text-red me-1 disabled' data-tippy-content='" . _("Private key is missing") . "' href='#'>{$dl_icon} .key</a>";
            }
            if (!empty($c->cert_id)) {
