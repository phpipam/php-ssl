...
try {
	...
	$query_all .= "select count(*) as cnt from hosts h where h.z_id = :zone_id ";
	...
	// search ?
	if(strlen($_POST['search'])>0) {
		$query         .= " and (h.hostname like :search or h.ip like :search)";
		$query_all     .= " and (h.hostname like :search or h.ip like :search)";
	...
	if(sizeof($hosts)>0) {
		foreach ($hosts as $h) {
			...
			if($cert_parsed['serialNumberHex']!="/") {
				$h->serial_html = "<a class='btn btn-sm text-$danger_class' href='/".$h->tenant_href."/certificates/".$h->zone_name."/".$cert_parsed['serialNumber']."/' target='_blank'>".$cert_parsed['serialNumberHex']."</a><br>".$status['text']." <span class='text-muted' style='font-size:11px;'>".$cert_parsed['custom_validTo']."</span>";
			}
			else {
			...
			// port column
			$h->port_html = "<span class='badge'>".$h->port_formatted."</span><br>".$h->tls_formatted;
