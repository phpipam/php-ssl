...
if (empty($csr_pem) || empty($key_pem)) {
    print json_encode(['status' => 'error', 'message' => _("CSR and private key are required.")]);
    exit;
}
...
// Parse and validate private key (supports encrypted keys when passphrase provided)
...
// Determine key algo and size
if (($csr_pub_details['type'] ?? -1) === OPENSSL_KEYTYPE_EC) {
    $key_algo = 'EC';
    $curve    = $csr_pub_details['ec']['curve_name'] ?? 'prime256v1';
    $key_size = ($curve === 'secp384r1') ? 384 : 256;
} else {
    $key_algo = 'RSA';
    $key_size = $csr_pub_details['bits'] ?? 2048;
}
...
// Insert CSR record
$Database->runQuery(
    "INSERT INTO csrs (t_id, cn, sans, key_algo, key_size, country, state, locality, org, ou, email, status, csr_pem, extensions, pkey_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)",
    [
        $t_id, $cn,
...
if ($cert_parsed !== null && $zone !== null) {
    ...
    // No cert provided — scan certificates for a key match
