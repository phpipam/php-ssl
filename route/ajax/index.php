...
if (empty($groups)) {
    ...
    foreach ($groups as $tenant_id => $cas) {
        ...
        if (empty($cas)) {
            print "<tr><td colspan='6'><div class='alert alert-info py-2 mb-0'>" . _("No CAs for this tenant.") . "</div></td></tr>";
            continue;
        }
        ...
        foreach ($cas as $ca) {
            ...
            $can_manage = (int)$user->permission >= 3;
            ...
            print "  <td>{$ca_icon} <a href='/route/modals/cas/view.php?ca_id={$ca_id}' class='text-body' data-bs-toggle='modal' data-bs-target='#modal1'>" . htmlspecialchars($ca->name) . "</a></td>";
