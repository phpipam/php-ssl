...
if($user->admin=="0") {
print '<a href="/route/modals/agents/edit.php?action=add&tenant='.$user->t_id.'" data-bs-toggle="modal" class="btn btn-sm btn-outline-success '.$user->actions_disabled.'"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-2"><path d="M12 5l0 14"></path><path d="M5 12l14 0"></path></svg> '._("New agent").'</a>';
}
print '</div>';

print '<div style="float:right">';
print '<a href="https://github.com/phpipam/php-ssl-agent" class="btn btn-5 align-right" target="_blank" rel="noreferrer">
	...
	foreach ($agent_groups as $tenant_id=>$group) {
		...
		if(sizeof($group)==0) {
			...
			foreach ($group as $a) {
				...
				print '	<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-device-desktop-search text-muted"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M11.5 16h-7.5a1 1 0 0 1 -1 -1v-10a1 1 0 0 1 1 -1h16a1 1 0 0 1 1 1v6.5" /><path d="M7 20h4" /><path d="M9 16v4" /><path d="M15 18a3 3 0 1 0 6 0a3 3 0 1 0 -6 0" /><path d="M20.2 20.2l1.8 1.8" /></svg>';
				print " ".$a->name."</td>";
				print "	<td>$status</td>";
				...
				print "		<a href='/route/modals/agents/edit.php?id=".$a->id."&action=edit&tenant=".$a->t_id."' data-bs-toggle='modal' data-bs-target='#modal1' class='".$user->actions_disabled."'>";
				...
				print "		<span class='badge text-light'>";
