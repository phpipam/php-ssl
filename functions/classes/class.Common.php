...
class Common extends Validate
{
	...
	public function print_system_warnings(): void
	{
		...
		if (!isset($installed) || $installed !== true) {
			$warnings[] = ['text' => 'Application is not marked as installed. Open <code style="font-size:10px;">config.php</code> and set <code style="font-size:10px;">$installed = true;</code>'];
		}
		...
		if (isset($Migration) && isset($user->admin) && $user->admin === '1') {
			...
			if (!empty($pending)) {
				...
				$btn     = "{$label}<button type='button' class='btn btn-sm btn-warning' id='btn-apply-migrations'>".'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-settings"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M10.325 4.317c.426 -1.756 2.924 -1.756 3.35 0a1.724 1.724 0 0 0 2.573 1.066c1.543 -.94 3.31 .826 2.37 2.37a1.724 1.724 0 0 0 1.065 2.572c1.756 .426 1.756 2.924 0 3.35a1.724 1.724 0 0 0 -1.066 2.573c.94 1.543 -.826 3.31 -2.37 2.37a1.724 1.724 0 0 0 -2.572 1.065c-.426 1.756 -2.924 1.756 -3.35 0a1.724 1.724 0 0 0 -2.573 -1.066c-1.543 .94 -3.31 -.826 -2.37 -2.37a1.724 1.724 0 0 0 -1.065 -2.572c-1.756 -.426 -1.756 -2.924 0 -3.35a1.724 1.724 0 0 0 1.066 -2.573c-.94 -1.543 .826 -3.31 2.37 -2.37c1 .608 2.296 .07 2.572 -1.065" /><path d="M9 12a3 3 0 1 0 6 0a3 3 0 0 0 -6 0" /></svg>'."Upgrade database</button>";
				$js      = "<script>document.getElementById('btn-apply-migrations').addEventListener('click',function(){var btn=this;btn.disabled=true;btn.textContent='Applying...';fetch('/route/ajax/apply-migrations.php',{method:'POST',headers:{'X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(function(d){if(d.success){location.reload();}else{var errs=(d.results||[]).filter(function(r){return !r.success;}).map(function(r){return r.file+': '+r.error;}).join('\\n');btn.disabled=false;btn.textContent='Failed — see details';alert('Migration failed:\\n'+errs);}}).catch(function(){btn.disabled=false;btn.textContent='Error — check console';});});</script>";
