...
class ScanThread  {
	...
	*/
    protected $errors = array(
        ScanThread::FUNCTION_NOT_CALLABLE => 'You must specify a valid function name that can be called from the current scope.',
        ScanThread::COULD_NOT_FORK => 'pcntl_fork() returned a status of -1. No new process was created',
    );
	...
	*/
    public function setRunnable( $_runnable ) {
        if( self::runnableOk( $_runnable ) ) {
            ...
            throw new Exception( $this->getError( ScanThread::FUNCTION_NOT_CALLABLE ), ScanThread::FUNCTION_NOT_CALLABLE );
        }
    }
	...
	*/
    public function start() {
        $pid = @ pcntl_fork();
        if( $pid == -1 ) {
            throw new Exception( $this->getError( ScanThread::COULD_NOT_FORK ), ScanThread::COULD_NOT_FORK );
        }
        if( $pid ) {
            // parent
            $this->pid = $pid;
