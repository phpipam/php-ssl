...
class URL extends Common
{
	...
	private function process_path($path = "")
	{
		...
		// save
		if (isset($path_arr[0])) {
			$this->uri_params['tenant'] = strip_tags($path_arr[0]);
		}
		if (isset($path_arr[1])) {
			$this->uri_params['route'] = strip_tags($path_arr[1]);
		}
		if (isset($path_arr[2])) {
			$this->uri_params['app'] = strip_tags($path_arr[2]);
		}
		if (isset($path_arr[3])) {
			$this->uri_params['id1'] = strip_tags($path_arr[3]);
		}
		if (isset($path_arr[4])) {
			$this->uri_params['id2'] = strip_tags($path_arr[4]);
		}
		if (isset($path_arr[5])) {
			$this->uri_params['id3'] = strip_tags($path_arr[5]);
		}
		// filter empty
